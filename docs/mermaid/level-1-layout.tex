...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[entity] (smtp) at (13.45,15.45) {SMTP Relay};
  \node[entity] (dns) at (13.45,11.75) {Public DNS Resolvers};
  \node[entity] (admin) at (13.45,4.65) {System Administrator};
  \node[entity] (osv) at (13.45,6.95) {OSV / EPSS Advisory APIs};
  \node[entity] (target) at (13.45,2.05) {Target Website};
  ...
  \draw[flow]
    (p07.south) -- (2.55,1.15) -- (15.55,1.15) -- (15.55,16.55)
    -- node[flowlabel,left,pos=0.45] {reports} (user.east);

  % Scope Guard and safety routing.
  \draw[flow]
    (p04.south east) -- (10.35,11.10) -- (15.25,11.10) -- (15.25,5.45)
    -- node[flowlabel,above,pos=0.45] {crawl candidate} (p08.north east);
  \draw[flow] (p05.south) -- node[flowlabel,right] {safe-test candidate} (p08.north);
  \draw[flow]
    (admin.west) -- node[flowlabel,above] {admin controls} (p08.east);
  \draw[flow] (p08.south east) -- node[flowlabel,below] {approved traffic} (target.west);

  % Third-party interactions.
  \draw[flow]
    (p02.south east) -- (4.55,14.55) -- node[flowlabel,above]
    {DNS TXT verification query} (10.95,14.55) -- (dns.north west);
  \draw[flow]
    (dns.west) -- (10.65,14.10) -- node[flowlabel,below]
    {DNS TXT verification result} (4.80,14.10) -- (p02.south east);

  \draw[flow] (p01.east) -- node[flowlabel,above] {verification email} (smtp.west);
  \draw[flow] (smtp.south west) -- node[flowlabel,below] {delivery status} (p01.south east);
