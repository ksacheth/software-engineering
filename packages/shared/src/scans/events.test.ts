...
describe("event names", () => {
  test("the SRS names are used verbatim", () => {
    expect([...SCAN_EVENT_TYPES].sort() as string[]).toEqual(
      [...SRS_EVENT_NAMES].sort(),
    );
  });

  test("scan.completed is not a scan event", () => {
    expect(
      isScanEvent({ type: "scan.completed", scanJobId: SCAN_ID, at: AT }),
    ).toBe(false);
  });
});
...
describe("malformed input is refused", () => {
  ...
  const bad: [string, unknown][] = [
    ...
    [
      "a numeric scanJobId",
      { type: "scan.status", scanJobId: 7, at, status: "RUNNING" },
    ],
    [
      "a missing timestamp",
      { type: "scan.status", scanJobId: SCAN_ID, status: "RUNNING" },
    ],
    [
      "a non-UTC timestamp",
      {
        type: "scan.status",
        scanJobId: SCAN_ID,
        at: "2026-09-11T09:15:00+02:00",
        status: "RUNNING",
      },
    ],
    [
      "a nonsense timestamp",
      { type: "scan.status", scanJobId: SCAN_ID, at: "yesterday" },
    ],
    [
      "an unknown status",
      { type: "scan.status", scanJobId: SCAN_ID, at, status: "STALLED" },
    ],
    [
      "a status without a status field",
      { type: "scan.status", scanJobId: SCAN_ID, at },
    ],
    [
    ...
    [
      // An empty fingerprint is not a cosmetic defect: the live view keys its
      // finding map on it, so accepting one would collapse a scan's findings
      // onto a single entry.
      "a finding with an empty fingerprint",
      {
        type: "scan.finding",
        scanJobId: SCAN_ID,
        at,
